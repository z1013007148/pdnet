...
def pdb2dmap(pdbfile):
    ...
    cb_map = np.full((len(xyz_cb), len(xyz_cb)), np.nan)
    for r1 in sorted(xyz_cb):
        (a, b, c) = xyz_cb[r1]
        for r2 in sorted(xyz_cb):
            (p, q, r) = xyz_cb[r2]
            cb_map[r1 - 1 - (L-len(xyz_cb)), r2 - 1 - (L-len(xyz_cb))] = sqrt((a-p)**2+(b-q)**2+(c-r)**2)
...
def disteval_main(native=None,
    ...
    native_name = os.path.split(native)[-1].replace(".pdb", "")
    dist_name = os.path.split(trrosetta)[-1].replace(".npz", "")
    f = open(os.path.join("outputs", "train_output", native_name+"_"+dist_name+".txt"), "a")
    ...
    if (ND is not None) and (D is not None):
        ...
        for LDDT_k, LDDT_v in LDDT_dict.items():
            ...
            f.write(str(LDDT_k)+"\t"+"Cb-LDDT:"+"\t"+str(LDDT_v)+"\n")
    f.close()
    if not modeling3d:
        sys.exit()
...
if __name__ == "__main__":
    ...
    dic = {}
    with open("/home/data/user/guangxingh/zpx/FLEXpdb_database/seq_contains_pairs.txt") as f:
    ...
    folder = "/home/data/user/guangxingh/zpx/DeepAccNet_mod/outputs/train"
    files = os.listdir(folder)
    for file in files:
        if "features" not in file:
            p_name = file.replace(".npz", "")
            native_path = os.path.join("train_set_pdb", p_name+".pdb")
            pool.apply_async(disteval_main, (native_path, None, None, os.path.join(folder, file)))
            native_path = os.path.join("train_set_pdb", dic[p_name] + ".pdb")
            pool.apply_async(disteval_main, (native_path, None, None, os.path.join(folder, file)))
    pool.close()
    pool.join()
